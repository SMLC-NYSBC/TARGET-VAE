...
def main():
    ...
    parser.add_argument('--encoder-kernel-size', type=int, default=64, help='size of kernels in the first layer of the encoder (default: 28)')
    ...
    parser.add_argument('--generator-num-layers', type=int, default=4, help='number of hidden layers (default: 2)')
    ...
    experiment_description = '_'.join([str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M'))
                                       , 'galaxy', 'zDim', str(z_dim),'translation', translation_inference, 'rotation'
                                       , rotation_inference])
    if group_conv > 0:
